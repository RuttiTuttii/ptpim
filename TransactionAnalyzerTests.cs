...
public class TransactionAnalyzerTests
{
    ...
    public void AllowsVipToTransferLargeExternal()
    {
        var tx = new Transaction
        {
            ...
            IsInternal = false,
            IsVipClient = true,
            DailyTransactionTotal = 0 // избегаем проверки дневного лимита
        };
    ...
    public void RejectsLargeExternalTransferForNonVip()
    {
        var tx = new Transaction
        {
            ...
            IsInternal = false,
            IsVipClient = false,
            DailyTransactionTotal = -1_000_001 // избегаем проверки дневного лимита
        };
    ...
    public void RejectsLargeExternalWithdrawalForNonVip()
    {
        ...
        Assert.StartsWith("Ограничение: крупное снятие", TransactionAnalyzer.AnalyzeTransaction(tx));
    }
    ...
    public void RejectsWebWithdrawal()
    {
        ...
        Assert.StartsWith("Отклонено: снятие через веб-банкинг запрещено.", TransactionAnalyzer.AnalyzeTransaction(tx));
    }
    ...
    {
        var weekend = new DateTime(2024, 1, 6); // Суббота
        var tx = new Transaction
        {
            Amount = 1000,
            Kind = TransactionKind.Transfer,
            ...
            Channel = "Mobile",
            Timestamp = weekend,
            IsVipClient = true, // VIP чтобы избежать комиссии
            IsInternal = true // внутренняя чтобы избежать комиссии
        };

        Assert.StartsWith("Ошибка: в выходные нельзя переводить между разными типами счетов через онлайн-каналы.", TransactionAnalyzer.AnalyzeTransaction(tx));
    }
    ...
    public void RejectsWeekendOnlineDifferentAccountTransfer()
    {
        ...
        Assert.StartsWith("Ошибка: в выходные нельзя", TransactionAnalyzer.AnalyzeTransaction(tx));
    }
    ...
    {
        var weekday = new DateTime(2024, 1, 3); // Среда
        var tx = new Transaction
        {
            Amount = 1000,
            Kind = TransactionKind.Transfer,
    ...
    public void AllowsInternalLargeTransferForNonVip()
    {
        var tx = new Transaction
        {
            ...
            IsInternal = true,
            IsVipClient = false,
            DailyTransactionTotal = -2_000_000 // избегаем проверки дневного лимита
        };
    ...
    public void AllowsWeekendOfficeDifferentAccountInternalTransfer()
    {
        ...
        var tx = new Transaction
        {
            ...
            Timestamp = weekend,
            IsVipClient = false,
            IsInternal = true // внутренняя транзакция - нет комиссии
        };
    ...
    public void AllowsWeekdayOnlineDifferentAccountVipTransfer()
    {
        ...
        var tx = new Transaction
        {
            ...
            Channel = "Web",
            Timestamp = weekday,
            IsVipClient = true, // VIP - нет комиссии
            IsInternal = false
        };
